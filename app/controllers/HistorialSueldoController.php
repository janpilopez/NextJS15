...
use Yii;
use app\models\SysAdmHistorialSueldo;
use app\models\Search\SysAdmHistorialSueldoSearch;
...
/**
 * HistorialSueldoController implements the CRUD actions for SysAdmHistorialSueldo model.
 */
class HistorialSueldoController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    /**
     * Lists all SysAdmHistorialSueldo models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new SysAdmHistorialSueldoSearch();
    ...
    /**
     * Displays a single SysAdmHistorialSueldo model.
     * @param string $id
     * @return mixed
    ...
    /**
     * Creates a new SysAdmHistorialSueldo model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate()
    {
        $model = new SysAdmHistorialSueldo();
        $model->anio = date('Y');
        
        if ($model->load(Yii::$app->request->post())) {
            
            $objSueldo = $this->findModel($model->anio);
            
            if (!$objSueldo):
            
              $model->user_created = Yii::$app->user->username;
              $model->date_created = date('Ymd H:i:s');
              $model->activo = false;
              $model->save(false);
              ...
              'icon' => 'glyphicons glyphicons-robot',
              'message' => 'El período '.$model->anio.' ya se encuentra registrado!',
              'positonY' => 'top','positonX' => 'right']);
    ...
    /**
     * Updates an existing SysAdmHistorialSueldo model.
    ...
    /**
     * Deletes an existing SysAdmHistorialSueldo model.
    ...
    private  function ActualizarSueldoEmpleados($model){
        ...
        $datos = Yii::$app->$db->createCommand("exec [dbo].[ObtenerEmpleadosXSueldoSectorizar]  @sueldo_sectorial = {$sueldoAnt->sueldo_sectorial}")->queryAll();
    ...
    /**
     * Finds the SysAdmHistorialSueldo model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return SysAdmHistorialSueldo the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($anio)
    {
        if (($model = SysAdmHistorialSueldo::findOne($anio)) !== null) {
            return $model;
        }
