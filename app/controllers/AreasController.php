...
use app\models\search\SysAdmAreasSearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;

/**
 * AreasController implements the CRUD actions for SysAdmAreas model.
 */
class AreasController extends Controller
{
    ...
    {
        $searchModel = new SysAdmAreasSearch();
    ...
    {
        $model = new SysAdmAreas();

        if ($model->load(Yii::$app->request->post())) {
            
            $codigo = SysAdmAreas::find()->select(['max(CAST(id_sys_adm_area AS INT))'])->Where(['id_sys_empresa'=> '001'])->scalar();
            
            $model->id_sys_adm_area = $codigo + 1;
