...
function gethora_entrada($cedula){
        
    $id_horario = SysRrhhEmpleadosHorario::find()->where(['id_sys_rrhh_cedula'=>$cedula])->one();
      
    if($id_horario){
        $horario = SysRrhhHorarioCab::find()->where(['id_sys_rrhh_horario_cab'=>$id_horario->id_sys_rrhh_horario])->one();
        return date('H:i:s', strtotime($horario->hora_inicio));
    }else{
        return '08:30:00';
    }
...
function getDatosPermisos($fechaini,$fechafin,$cedula){
        
    $db =  $_SESSION['db'];
         
    $datos = Yii::$app->$db->createCommand("EXEC [dbo].[ObtenerPermisosXCedula]  @fechaini = '{$fechaini}' , @fechafin = '{$fechafin}', @cedula = '{$cedula}', @tipo = 'W'")->queryAll(); 
    return $datos;
    
}

function gethora_agendamiento($agendamiento,$fechaentrada,$cedula){
    
    $db =  $_SESSION['db'];
         
    $datos = Yii::$app->$db->createCommand("EXEC [dbo].[ObtenerAsistenciaAgendamiento]  @jornada = $agendamiento , @fecha = '$fechaentrada', @cedula = '$cedula'")->queryOne(); 
    return $datos;
}
...
function DecimaltoHorasComedor($valor){
    ...
    if($valor > 0):
        ...
        return  str_pad($h, 2, "0", STR_PAD_LEFT).':'.str_pad(intval(round($m)), 2, "0", STR_PAD_LEFT).':'.str_pad(intval(round($s)), 2, "0", STR_PAD_LEFT).'';
...
function HorasToDecimalComedor($hora){
