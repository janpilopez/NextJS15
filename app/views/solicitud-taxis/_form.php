...
use app\models\SysRrhhsotaxisEmpleados;
use app\assets\SotaxisAsset;
use yii\web\View;

SotaxisAsset::register($this);
use yii\web\JsExpression;
$url = Yii::$app->urlManager->createUrl(['solicitud-taxis']);
...
$cont =  SysRrhhSolicitudTaxisEmpleados::find()->where(['id_sys_rrhh_sotaxis'=> $model->id_sys_rrhh_sotaxis, 'id_sys_empresa'=> $model->id_sys_empresa])->count();

$iddetalle =
[
    'name' => 'id_sys_rrhh_sotaxis_emp',
...
?>
<div class="sys-rrhh-sotaxis-form">
 <?php $form = ActiveForm::begin(['id'=>'sotaxisemp']); ?>
 <div class = 'row'>
    <div class = 'col-md-3'>                
        <?= $form->field($model, 'fecha_solicitada')->widget(DatePicker::classname(), [
            'removeButton' => false,
            'size'=>'md',
            'options' => ['placeholder' => 'Día Solicitado','id'=>'fecha_solicitada','class'=> 'form-control input-sm'],
            'pluginOptions' => $pluginOptions,
            ]);?>
    </div>
     <div class = 'col-md-2'>
         <?= $form->field($model, 'id_sys_adm_area')->dropDownList(ArrayHelper::map(SysAdmAreas::find()->andFilterWhere(['id_sys_adm_area'=> $areas])->all(), 'id_sys_adm_area', 'area'), ['prompt'=> 'Todos' ,'class'=> 'form-control input-sm', 'id'=>'area']  ) ?> 
     </div>
      <div class = 'col-md-3'>
    ...
    <div class = 'row'>
        <div class = 'col-md-3'>
        <?=  $form->field($model, 'telefono')->widget(NumberControl::classname(), [
                        'displayOptions' =>  [
                            'placeholder' => '',
                            'class'=> 'form-control'
                        ],
  ...
  <div class= 'row'>
     <div class= 'col-md-12'>
         <?= Html::hiddenInput('sotaxisemp', $cont, ['id'=> 'datasotaxis']); ?>
         <?php 
