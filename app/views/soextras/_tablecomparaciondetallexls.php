...
if($datos):
    ...
    foreach ($dataFilterIdSysAdmArea as $index => $departamento):
        ...
        if($area != ""):
                ...
                $hojita->setCellValue('A'.$i, 'Cédula');
                $hojita->getStyle('A'.$i)->getFont()->setBold(true);
                $hojita->setCellValue('B'.$i, 'Nombres');
                $hojita->getStyle('B'.$i)->getFont()->setBold(true);
                $hojita->setCellValue('C'.$i, 'Fecha');
                $hojita->getStyle('C'.$i)->getFont()->setBold(true);
                ...
                foreach ($dataFilterIdCedula as $index => $row):
                    ...
                    foreach($newData as $new):
                        ...
                        //if($new['solh50'] != null){
                            ...
                            $existe = true;
                            $h50  = getRendonminutos(gethoras50(date("Y-m-d H:i:s",strtotime($new['entrada'])),  date("Y-m-d H:i:s",strtotime($new['salida'])),$new['id_sys_rrhh_cedula'], $new['fecha'], $new['feriado']));
                            $h100  = getRendonminutos(gethoras100(date("Y-m-d H:i:s",strtotime($new['entrada'])),  date("Y-m-d H:i:s",strtotime($new['salida'])),$new['id_sys_rrhh_cedula'], $new['fecha'], $new['feriado'],$new['agendamiento']));
                            ...
                            $hojita->setCellValue('A'.$i, $new['id_sys_rrhh_cedula']);
                            $hojita->setCellValue('B'.$i, $new['nombres']);
                            $hojita->setCellValue('C'.$i, $new['fecha']);
                            $hojita->setCellValue('D'.$i, $h50);
                            $hojita->setCellValue('E'.$i, DecimaltoHoras(number_format($new['solh50'], 2, '.', '')));
                            $hojita->setCellValue('F'.$i, DecimaltoHoras(number_format($new['solA50'], 2, '.', '')));
                            $hojita->setCellValue('G'.$i, $h100);
                            $hojita->setCellValue('H'.$i, DecimaltoHoras(number_format($new['solh100'], 2, '.', '')));
                            $hojita->setCellValue('I'.$i, DecimaltoHoras(number_format($new['solA100'], 2, '.', '')));
                ...
                if($existe == true){
                    ...
                    $hojita->setCellValue('A'.$i, "TOTAL");
                    $hojita->getStyle('A'.$i)->getFont()->setBold(true);
                    $hojita->mergeCells('A'.$i.':C'.$i.'');
                    $hojita->setCellValue('D'.$i, DecimaltoHoras(number_format($TotalH50, 2, '.', '')));
                    $hojita->setCellValue('E'.$i, DecimaltoHoras(number_format($hT50, 2, '.', '')));
                    $hojita->setCellValue('F'.$i, DecimaltoHoras(number_format($aT50, 2, '.', '')));
                    $hojita->setCellValue('G'.$i, DecimaltoHoras(number_format($TotalH100, 2, '.', '')));
                    $hojita->setCellValue('H'.$i, DecimaltoHoras(number_format($hT100, 2, '.', '')));
                    $hojita->setCellValue('I'.$i, DecimaltoHoras(number_format($aT100, 2, '.', '')));
