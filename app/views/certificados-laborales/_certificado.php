...
<div class= "row">
  <div class = "col-md-12">
      <table style= "width: 100%;">
          <tr>
            <td width = "50%" rowspan="25" style="text-align:right;"><img src=" <?= Yii::getAlias('@webroot')."/".trim("logo/".$empresa->ruc."/".$empresa->logo)?>" height="60" width="240" class ="text-center"></td>
          </tr>
      </table>
  </div>
</div>
<div class = "row margen-left">
   <div class = "col-md-12 margen-left">
     ...
     <br>
     <p style ="text-align: center;font-size: 20px;font-weight: bold;">CERTIFICACIÓN LABORAL</span></p>
     <br>
     <br>
     ...
     if($contrato){
     ?> 
     <p style ="text-align: justify;">Tengo a bien certificar que el/la Sr./Sra./Srta. <span style="font-weight: bold;"><?= strtoupper($empleado->nombres)?></span>, con cédula de ciudadanía número <?= $empleado->id_sys_rrhh_cedula?>, labora en la empresa <?= $empresa->razon_social?> desde el <?= date("d", strtotime($contrato->fecha_ingreso))?> de <?= $meses[date("n", strtotime($contrato->fecha_ingreso))]?> del <?= date("Y", strtotime($contrato->fecha_ingreso))?> hasta la actualidad,
      desempeñando el cargo de <span style="font-weight: bold;"><?= ($cargo->cargo)?></span>, percibiendo una remuneración mensual de $<?= $numero_con_decimales=floatval($sueldo->sueldo); $son=number_format($numero_con_decimales,2,".",""); $V=new EnLetras();?> ( <?php echo $V->ValorEnLetras($son,"d&oacute;lares")?>).</p>
     <?php }else{
        if($empleado->nivel_riesgo == 1){
     ...
     <?php }else{
      ?><p style ="text-align: justify;">Tengo a bien certificar que el/la Sr./Sra./Srta. <span class='negrita'><?= strtoupper($empleado->nombres)?></span>, con cédula de ciudadanía número <?= $empleado->id_sys_rrhh_cedula?>, laboró en la empresa <?= $empresa->razon_social?> como <?= ($cargo->cargo)?>, desde el <?= date("d", strtotime($contratoSalida[0]['fecha_ingreso']))?> de <?= $meses[date("n", strtotime($contratoSalida[0]['fecha_ingreso']))]?> del <?= date("Y", strtotime($contratoSalida[0]['fecha_ingreso']))?> 
      hasta el <?= date("d", strtotime($contratoSalida[0]['fecha_salida']))?> de <?= $meses[date("n", strtotime($contratoSalida[0]['fecha_salida']))]?> del <?= date("Y", strtotime($contratoSalida[0]['fecha_salida']))?>.</p> 
     <?php }
    }?> 
     <br>
    ...
    <table style= "width: 100%;">
        <tr>
         <td width = "50%" style="text-align:center;"><img src=" <?= Yii::getAlias('@webroot')."/".trim("img/firmacertificado.png")?>" height="100" width="200" class ="text-center"></td>
        </tr>
        <tr>
         <td width = "50%" style="text-align:center;"><p style="font-weight: bold;">Ab. Soledad Orbes</p></td>
        </tr>
        <tr>
         <td width = "50%" style="text-align:center;"><p style="font-weight: bold;">Jefe de Talento Humano</p></td>
        </tr>
        <tr>
         <td width = "50%" style="text-align:center;"><p style="font-weight: bold;"><?= $empresa->razon_social?></p></td>
        </tr>
    </table>
</div>
